...
const bookAppointmentCallback = async (query: Record<string, any>) => {
    ...
    const transactionResult = await prisma.$transaction(async (tx) => {
        ...
        if (status === "success") {
            ...
            // (total - available) + 1 serial number
            ...
            //? get joining time based on serial number and startDateTime
            const joiningTime = addMinutes(
                appointment.schedule.startDateTime,
                (serialNumber - 1) * 20
            )

            //? update appointment status, joining time and serial number
            ...
            //? update schedule available slots
            ...
            //? on end event listener for pdf document end event listener
...
const cancelAppointment = async (payload: ICancelAppointmentPayload, user: RequestUser) => {
    ...
    const transactionResult = await prisma.$transaction(async (tx) => {
        ...
        //? get new payment info after refund
...
//* Update Appointment Status for Doctor Only (Confirmed => ONGOING => COMPLETED)  
...
const getMyAppointments = async (query: IQuery, user: RequestUser) => {
    ...
    //? default where condition for patientId
...
//* Get All Appointments for admin super admin
const getAllAppointments = async (query: IQuery) => {
    ...
    //? find all appointments with pagination, sorting and filtering
    const appointments = await prisma.apppointment.findMany({
        where: { AND: andConditions },
        take: limit,
        skip,
...
//* Get Single Appointment for all loggedin user
